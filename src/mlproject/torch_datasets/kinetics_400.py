...
TEST_DOWNLOAD_GB = 55  # more precisely, 52
TEST_EXTRACT_GB = 55  # more precisely, 52
...
def download_kinetics_test_set(dataset_rootdir: str | Path):
    # Check disk space
    dataset_dir = Path(dataset_rootdir)
    if dataset_dir.name != "kinetics-dataset":
        dataset_dir = dataset_dir / "kinetics-dataset"
    ...
    download_dir.mkdir(parents=True, exist_ok=True)
    disk_space = shutil.disk_usage(dataset_dir).free
    if disk_space < TEST_DOWNLOAD_GB * 1024 * 1024 * 1024:
        raise RuntimeError(
            f"Insufficient disk space. At least {TEST_DOWNLOAD_GB}GB is required, but only {disk_space / 1024 / 1024 / 1024:.1f} GB available"
        )
    ...
    snapshot_download(
        ...
        local_dir=download_dir,
        resume_download=True,
        allow_patterns=["test/*", "replacement/*"],
    )

    logger.info("Successfully downloaded kinetics-400 test set.")
...
def extract_kinetics_test_set(
    ...
    extract_dir.mkdir(parents=True, exist_ok=True)
    disk_space = shutil.disk_usage(extract_dir).free
    if disk_space < TEST_EXTRACT_GB * 1024 * 1024 * 1024:
        raise RuntimeError(
            f"Insufficient disk space. At least {TEST_EXTRACT_GB}GB is required, but only {disk_space / 1024 / 1024 / 1024:.1f} GB available"
        )
    ...
    logger.info("Successfully extracted kinetics-400 test set.")
...
def arrange_by_classes(dataset_rootdir: str | Path, splits: list[str] = SPLITS):
    dataset_dir = Path(dataset_rootdir)
    if dataset_dir.name != "kinetics-dataset":
        dataset_dir = dataset_dir / "kinetics-dataset"
...
def check_num_files_each_class(dataset_rootdir: str | Path, splits: list[str] = SPLITS):
    dataset_dir = Path(dataset_rootdir)
    if dataset_dir.name != "kinetics-dataset":
        dataset_dir = dataset_dir / "kinetics-dataset"
...
def prepare_kinetics_400_test_set(
    download_dataset_rootdir: str | Path,
    extract_dataset_rootdir: str | Path | None = None,
):
    try:
        # Check if everything is already prepared
        if extract_dataset_rootdir is None:
            check_num_files_each_class(download_dataset_rootdir, ["test"])
        else:
            check_num_files_each_class(extract_dataset_rootdir, ["test"])
    except FileNotFoundError:
        # Not prepared, so prepare it
        if extract_dataset_rootdir is None:
            download_dataset_rootdir = Path(download_dataset_rootdir)
            download_dataset_rootdir.mkdir(parents=True, exist_ok=True)
            disk_space = shutil.disk_usage(download_dataset_rootdir).free
            if disk_space < (TEST_DOWNLOAD_GB + TEST_EXTRACT_GB) * 1024 * 1024 * 1024:
                raise RuntimeError(
                    f"Insufficient disk space. At least {TEST_DOWNLOAD_GB+TEST_EXTRACT_GB}GB is required, but only {disk_space / 1024 / 1024 / 1024:.1f} GB available"
                )
        else:
            download_dataset_rootdir = Path(download_dataset_rootdir)
            extract_dataset_rootdir = Path(extract_dataset_rootdir)
            ...
            if (
                ...
                if (
                    disk_space
                    < (TEST_DOWNLOAD_GB + TEST_EXTRACT_GB) * 1024 * 1024 * 1024
                ):
                    raise RuntimeError(
                        f"Insufficient disk space. At least {TEST_DOWNLOAD_GB+TEST_EXTRACT_GB}GB is required, but only {disk_space / 1024 / 1024 / 1024:.1f} GB available"
                    )
        ...
        download_kinetics_test_set(download_dataset_rootdir)
        extract_kinetics_test_set(download_dataset_rootdir, extract_dataset_rootdir)

        if extract_dataset_rootdir is None:
            download_kinetics_annotations(download_dataset_rootdir)
            arrange_by_classes(download_dataset_rootdir, splits=["test"])
            check_num_files_each_class(download_dataset_rootdir, splits=["test"])
        else:
            download_kinetics_annotations(extract_dataset_rootdir)
            arrange_by_classes(extract_dataset_rootdir, splits=["test"])
            check_num_files_each_class(extract_dataset_rootdir, splits=["test"])
    else:
        logger.info("Kinetics-400 dataset is already prepared.")
        return
...
if __name__ == "__main__":
    ...
    prepare_kinetics_400_test_set(
        "/disk/scratch2/datasets/temp", "/disk/scratch_fast1/datasets/temp"
    )
