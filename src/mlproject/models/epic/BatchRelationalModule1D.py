...
class BatchRelationalModule(nn.Module):
    def __init__(
        self, input_feature_dim, use_coordinates=False, num_layers=2, num_units=64
    ):
    ...
    def build_block(self):
        print(
            "Assuming input is of size (b=4, num_object=4, input_feature_dim=%d)"
        ...
        for idx_layer in range(self.num_layers):
            self.block_dict["g_fcc_{}".format(idx_layer)] = nn.Linear(
                out.shape[1], out_features=self.num_units, bias=True
            )
        ...
        """f"""
        self.post_processing_layer = nn.Linear(
            in_features=out.shape[1], out_features=self.num_units
        )
    ...
    def forward(self, x_img):
        if isinstance(x_img, list):
                ...
                if self.use_coordinates:
                    if self.coord_tensor.shape[0] != out_img.shape[0]:
                        self.coord_tensor = (
                            self.coord_tensor[0]
                            .unsqueeze(0)
                            .repeat([out_img.shape[0], 1, 1])
                        )

                    # print(self.coord_tensor)
                    out_img = torch.cat(
                        [out_img, self.coord_tensor.to(x_img1.device)], dim=2
                    )
                # x_flat = (64 x 25 x 24)
            ...
            for b in range(batch_size):
                ...
                out_list[b] = out[out_idx : out_idx + length_per_batch[b] ** 2].view(
                    length_per_batch[b], length_per_batch[b], -1
                )
            ...
            if self.use_coordinates:
                ...
                # print(self.coord_tensor)
                out_img = torch.cat(
                    [out_img, self.coord_tensor.to(x_img.device)], dim=2
                )
            # x_flat = (64 x 25 x 24)
            # print('out_img', out_img.shape)
            x_i = torch.unsqueeze(out_img, 1)  # (1xh*wxc)
