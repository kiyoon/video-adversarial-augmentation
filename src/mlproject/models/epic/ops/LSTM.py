...
class LSTMSequenceEncoder(nn.Module):
    def __init__(
        self, feature_dim, hidden_dim, num_layers, bidirectional=True, dropout=0.5
    ):
        """
        Params:
...
class LSTMClassifier(nn.Module):
    def __init__(
        ...
        self.encoder = LSTMSequenceEncoder(
            feature_dim, lstm_hidden_dim, lstm_layers, lstm_bidirectional, lstm_dropout
        )

        self.output_layers = nn.Sequential(
            nn.Linear(
                2 * lstm_hidden_dim if lstm_bidirectional else lstm_hidden_dim,
                lstm_hidden_dim,
            ),
            nn.ReLU(),
        ...
        if attention:
            self.attention_layer = nn.Linear(
                2 * lstm_hidden_dim if lstm_bidirectional else lstm_hidden_dim, 1
            )
...
if __name__ == "__main__":
    ...
    model = LSTMClassifier(
        num_classes, feature_dim, nlayers, hidden_dim, dropout, True, True
    )
