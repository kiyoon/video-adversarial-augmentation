...
def make_multi_view_fusion(
    net, n_segment, alpha, mvf_freq=(1, 1, 1, 1), use_hs=True, share=False, mode="THW"
):
    ...
    if len(list(net.layer3.children())) >= 23:  # R101 & R152
        # n_round = 2
        logger.info("=> Using n_round {} to insert MVF module".format(n_round))

    def make_block_MVF(stage, this_segment):
        """build MVF Block"""
        blocks = list(stage.children())
        logger.info(
            "=> Processing stage with {} {} blocks residual".format(len(blocks), mode)
        )
    ...
    net.layer1 = (
        make_block_MVF(net.layer1, n_segment_list[0]) if mvf_freq[0] else net.layer1
    )
    net.layer2 = (
        make_block_MVF(net.layer2, n_segment_list[1]) if mvf_freq[1] else net.layer2
    )
    net.layer3 = (
        make_block_MVF(net.layer3, n_segment_list[2]) if mvf_freq[2] else net.layer3
    )
    net.layer4 = (
        make_block_MVF(net.layer4, n_segment_list[3]) if mvf_freq[3] else net.layer4
    )
...
class MVF(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.num_shift_channel != 0:
            x = x.view(n_batch, self.n_segment, c, h, w).transpose(
                1, 2
            )  # n, c, t, h, w
            ...
            if self.mode == "THW":
                ...
                if self.share:
                    ...
                    tmp_w = self.shift_conv(x[0].permute(0, 1, 4, 2, 3)).permute(
                        0, 1, 3, 4, 2
                    )
                else:
