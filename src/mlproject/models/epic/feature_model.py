...
class feature_model(nn.Module):
    ...
    def build_block(self):
        logger.info(
            "Assuming input is of size (b=4, input_feature_dim=%d)"
        ...
        if self.num_classes > 0:
            self.block_dict["classify_fc_{}".format(idx_layer)] = nn.Linear(
                out.shape[1], out_features=self.num_classes, bias=True
            )
