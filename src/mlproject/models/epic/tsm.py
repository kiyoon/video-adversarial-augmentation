...
from .torchvision_weights import get_torchvision_model, valid_torchvision_pretrained
...
class TSM(nn.Module):
    ...
    def __init__(
        ...
        self.flow_num_channels = flow_num_channels
...
TSM Configurations:
    ...
    dropout_ratio:      {}
        ...
        if partial_bn:
            self.partialBN(True)
        if pretrained and pretrained not in valid_torchvision_pretrained:
    ...
    def features(self, input: torch.Tensor) -> torch.Tensor:
        sample_len = (
            3 if self.modality == "RGB" else self.flow_num_channels
        ) * self.new_length
    ...
    def _get_diff(self, input, keep_rgb=False):
        input_c = 3 if self.modality in ["RGB", "RGBDiff"] else self.flow_num_channels
    ...
    def _construct_flow_model(self, base_model):
        ...
        new_kernel_size = (
            kernel_size[:1]
            + (self.flow_num_channels * self.new_length,)
            + kernel_size[2:]
        )
        new_kernels = (
            params[0]
        ...
        if self.base_model_name == "BNInception":
            ...
            if self.pretrained in valid_torchvision_pretrained:
                LOG.warning(
                    "#" * 30
                    + f"However, flow model is converted from {self.pretrained} pretrained RGB model."
                )
        return base_model
