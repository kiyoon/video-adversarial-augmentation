...
from mlproject.utils.loader import model_load_state_dict_nostrict
...
class Uniformer(nn.Module):
    ...
    def get_pretrained_model(self, pretrained_name="uniformer_small_in1k.pth"):
        model_path = hf_hub_download(
            repo_id="Sense-X/uniformer_image",
    ...
    def features(self, x):
        N, C, T, H, W = x.shape
        if C != self.in_chans and T == self.in_chans:
            # NTCHW -> NCTHW
            x = x.transpose(1, 2)
...
def uniformer_small(num_classes, pretrained="imagenet"):
    assert pretrained in [None, "imagenet"]
    model = Uniformer(
    ...
    if pretrained == "imagenet":
        checkpoint = model.get_pretrained_model()
        model_load_state_dict_nostrict(model, checkpoint)

    return model


def uniformer_base(num_classes):
    return Uniformer(
        num_classes=num_classes,
        depth=[5, 8, 20, 7],
        embed_dim=[64, 128, 320, 512],
...
if __name__ == "__main__":
    ...
    out = model(video)
    features = model.features(video)
    print(out.shape)  # (2, 400)
    print(features.shape)  # (2, 512)
