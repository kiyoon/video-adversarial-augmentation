...
def model_load_state_dict_partial(model, weights_state_dict):
    ...
    weights_state_dict_filtered = {
        k: v for k, v in weights_state_dict.items() if k in model_dict
    }
...
def model_load_state_dict_nostrict(model, weights_state_dict, partial=True):
    ...
    if is_changed:
        ...
        logger.warning(
            "Parameters have been changed. You may not want to use optimiser_state from the checkpoint"
        )
...
def model_load_weights_GPU(
    model, weights_path, cur_device=None, world_size=None, model_state_key="model_state"
):
...
def model_load_weights_GPU_nostrict(
    model,
    weights_path,
    cur_device=None,
    world_size=None,
